"""
Train a RealNVP normalizing flow to approximate a 4-dimensional correlated Gaussian,
then use Turing.jl to run NUTS inference under both the exact prior and the NF prior.

Usage:
    julia --project=. -t 4 examples/train_multinormal.jl
...
flow = FlowDistribution(Float64;
    n_transforms        = 6,
    dist_dims           = 4,
    hidden_layer_sizes  = [16, 16, 16],   # 3 hidden layers of 64 neurons each
    rng,
)

println("\nTraining RealNVP (6 transforms, [16, 16, 16] hidden units, 500 epochs)…")
...
println("Mean log-pdf (trained flow):      ", round(Float64(lp_flow); digits=4))
...
save_dir = joinpath(@__DIR__, "..", "trained_flows", "mvn_4d")
...
#
# Model: θ ~ prior,  y_obs ~ Normal(θ[1], 0.5)
# We infer θ from a single noisy observation of its first component,
# comparing the exact MvNormal prior vs the trained NF prior.
...
# ── 7b. Trained NF prior ─────────────────────────────────────────────────────
println("Sampling with trained NF prior ($n_chains chains × $n_samples samples)…")
chain_nf = sample(
...
@printf "  Posterior mean (NF):    %.4f  ± %.4f\n" mean(nf_θ1) std(nf_θ1)

println("\n✨ End of script: Turing MCMC sampling finished successfully!")
